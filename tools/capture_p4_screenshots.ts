/**
 * Deterministic SVG screenshots for P4 gate (initial / mid / final × accumulate + filter).
 * Coordinates come from the layout engine — never hand-placed.
 */
...
const outDir = join(root, 'BUILD-LOG', 'assets', 'p4');
...
function load(name: string) {
  ...
  return { graph, trace, scene: buildScene(graph, trace, { sceneId: name }) };
}

function svgFor(name: string, phase: 'initial' | 'mid' | 'final') {
  const { scene, graph, trace } = load(name);
  const stepIndex =
    phase === 'initial' ? 0 : phase === 'final' ? scene.steps.length - 1 : Math.floor(scene.steps.length / 2);
  const step = scene.steps[stepIndex]!;
  const focus = new Set(step.focus);
  const bindings = trace.steps[stepIndex]?.bindings ?? {};
  const width = Math.max(...scene.layout.map((n) => n.x + n.width)) + 28;
  const height = Math.max(...scene.layout.map((n) => n.y + n.height)) + 28;
  ...
  <text x="${n.x + 6}" y="${n.y + 18}" font-family="ui-monospace,monospace" font-size="11" fill="${stroke}">${escapeXml(label + value)}</text>
</g>`;
    })
    .join('\n');
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="12" y="16" font-family="ui-monospace,monospace" font-size="10" fill="#6F7280">${name} · ${phase} · step ${stepIndex}</text>
  ${rects}
</svg>
`;
}
...
for (const name of ['accumulate', 'filter'] as const) {
  for (const phase of ['initial', 'mid', 'final'] as const) {
    ...
    files.push(file);
    console.log(`wrote BUILD-LOG/assets/p4/${file}`);
  }
}

console.log(`${files.length}/6 screenshots saved`);
